...
def letters_by_format(df: pd.DataFrame):
    ...
    fig.add_trace(go.Scatter(x=x_vals_1_sqrt2, y=y_vals_1_sqrt2, mode="lines", line=dict(dash="dash", color="green", width=2), name="1/√2"))
...
def letters_by_size_only_letters_postcards_telegrams(df):
...
def main():
    ...
    df_2 = pd.read_csv(r"../eval/metadata_total.csv", sep=",", encoding="UTF-8")
    ...
    letters_by_color(df)
    letters_by_size_wo_overlay(df)
    letter_by_years(df)
    letters_by_size_only_letters_postcards_telegrams(df)
    page_fill(df)
